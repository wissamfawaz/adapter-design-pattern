...
function load() {
  ...
  storageLoop = setInterval(() => {

    if(events.length!=eventsSize){
      ...
      for(i=0; i<events.length; i++){
        ...
        if(events[i]["date"]== today){
          ...
          getTimeDifference(timeToMins(events[i]["time"]));
           prepareAddress(events[i]["locationName"], events[i]["city"], events[i]["country"]);
        }
